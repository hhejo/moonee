(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&o(l)}).observe(document,{childList:!0,subtree:!0});function r(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(n){if(n.ep)return;n.ep=!0;const a=r(n);fetch(n.href,a)}})();let[d,x,i]=[null,"accountBook","items"];function g(){let e=indexedDB.open(x,1);e.onerror=t=>console.error("Database error:",t.target.errorCode),e.onsuccess=t=>{d=t.target.result;let r=new CustomEvent("db-opened",{detail:{db:d}});document.dispatchEvent(r)},e.onupgradeneeded=t=>{if(d=t.target.result,!d.objectStoreNames.contains(i)){let r={keyPath:"id",autoIncrement:!0},o=d.createObjectStore(i,r);o.createIndex("date","date"),o.createIndex("content","content"),o.createIndex("price","price")}}}g();function c(){return new Promise((e,t)=>{let a=d.transaction(i,"readonly").objectStore(i).index("date").openCursor(null,"prev"),l=[],s="";a.onsuccess=y=>{let p=y.target.result;if(p){let m=p.value;s!==m.date&&l.push({date:m.date,price:"",content:"",id:-1}),l.push(m),s=m.date,p.continue()}else e(l)},a.onerror=y=>t(y)})}function v(e){return new Promise((t,r)=>{let a=d.transaction(i,"readwrite").objectStore(i).add(e);a.onsuccess=()=>t(),a.onerror=l=>console.error("Add error:",a.error)})}function h(e){return new Promise((t,r)=>{let a=d.transaction(i,"readwrite").objectStore(i).delete(e);a.onsuccess=()=>t(),a.onerror=l=>console.error("Delete error:",a.error)})}let w=document.getElementById("headerDiv");w.innerHTML=`
...
`;function u(e){let[t,r,o]=[0,0,0];for(let{id:l,price:s}of e)l!==-1&&(o+=s,s>=0?t+=s:r+=s);[t,r]=[t,r].map(Math.abs);let n=l=>new Intl.NumberFormat().format(l),a=document.getElementById("total");a.textContent=n(Math.abs(o)),document.getElementById("totalIncome").textContent=n(t),document.getElementById("totalExpense").textContent=n(r),o>0?(a.classList.remove("text-gray-500","text-red-500"),a.classList.add("text-sky-500")):o<0?(a.classList.remove("text-gray-500","text-sky-500"),a.classList.add("text-red-500")):(a.classList.remove("text-sky-500","text-red-500"),a.classList.add("text-gray-500"))}let I=document.getElementById("createItemFormSection"),E=`
  <form
    id="createItemForm"
...
`;j.innerHTML=S;function k(){let e=document.getElementById("createItemForm");e.classList.remove("hidden"),setTimeout(()=>{e.classList.remove("opacity-0"),e.classList.add("opacity-100")},20),document.getElementById("createItemFormButton").classList.add("hidden")}window.displayCreateItemFormHandler=k;function H(e){let t=document.createElement("h3");return t.className="w-full h-8 py-1 border-b text-right text-gray-400",[t.id,t.textContent]=[e,e],t}function $({id:e,date:t,price:r,content:o}){let n=document.createElement("li");[n.id,n.onclick]=[e,()=>void 0],n.className="flex w-full h-14 py-1 rounded-md transition duration-75 hover:bg-gray-50";let a=`text-${r>=0?r===0?"gray":"sky":"red"}-500`;return r=new Intl.NumberFormat().format(r<=0?-r:r),n.innerHTML=`
    <div class="flex flex-col w-full truncate">
      <span class="text-xs text-gray-300">${t}</span>
      <span class="flex items-center truncate text-gray-600 h-full">${o}</span>
    </div>
    <div class="flex justify-end items-center w-28">
      <span class="text-sm ${a}">${r}</span>
    </div>
  `,n}function f(e){const t=document.getElementById("itemList");for(;t.firstChild;)t.firstChild.remove();for(let r of e){if(r.id===-1){let n=H(r.date);t.appendChild(n);continue}let o=$(r);t.appendChild(o)}}function O(e){let t=e.target.textContent.trim(),r=new CustomEvent("item-filtered",{detail:{btnType:t}});document.dispatchEvent(r);let o=document.getElementById("allFilterBtn"),n=document.getElementById("incomeFilterBtn"),a=document.getElementById("expenseFilterBtn");switch(t){case"수입":o.classList.remove("text-gray-700","border-b-2","border-gray-700"),o.classList.add("text-gray-300"),n.classList.remove("text-gray-300"),n.classList.add("text-gray-700","border-b-2","border-gray-700"),a.classList.remove("text-gray-700","border-b-2","border-gray-700"),a.classList.add("text-gray-300");return;case"지출":o.classList.remove("text-gray-700","border-b-2","border-gray-700"),o.classList.add("text-gray-300"),n.classList.remove("text-gray-700","border-b-2","border-gray-700"),n.classList.add("text-gray-300"),a.classList.remove("text-gray-300"),a.classList.add("text-gray-700","border-b-2","border-gray-700");return;case"전체":o.classList.remove("text-gray-300"),o.classList.add("text-gray-700","border-b-2","border-gray-700"),n.classList.remove("text-gray-700","border-b-2","border-gray-700"),n.classList.add("text-gray-300"),a.classList.remove("text-gray-700","border-b-2","border-gray-700"),a.classList.add("text-gray-300");return}}window.clickFilterBtnHandler=O;function D(e,t){var o;if(e==="전체")return t;let r=[];for(let n of t){if(n.id===-1){((o=r.at(-1))==null?void 0:o.id)===-1&&r.pop(),r.push(n);continue}(e==="수입"&&n.price>0||e==="지출"&&n.price<0)&&r.push(n)}return r}function T(){let e=new CustomEvent("export-csv");document.dispatchEvent(e)}window.clickExportToCSVBtnHandler=T;function P(e){if(e=e.filter(s=>s.id!==-1),e.length===0)return;let t=Object.keys(e[0]).join(","),r=e.map(s=>Object.values(s).join(",")),o=[t,...r].join(`
`),n=new Blob([o],{type:"text/csv;charset=utf-8;"}),a=URL.createObjectURL(n),l=document.createElement("a");l.setAttribute("href",a),l.setAttribute("download","account_book.csv"),l.style.visibility="hidden",document.body.appendChild(l),l.click(),document.body.removeChild(l)}document.addEventListener("db-opened",async()=>{let e=await c();f(e),u(e)});document.addEventListener("item-added",async e=>{let{item:t}=e.detail;await v(t);let r=await c();f(r),u(r)});document.addEventListener("item-deleted",async e=>{let{id:t}=e.detail;await h(t);let r=await c();f(r),u(r)});document.addEventListener("item-filtered",async e=>{let{btnType:t}=e.detail,r=await c(),o=D(t,r);f(o),u(r)});document.addEventListener("export-csv",async e=>{let t=await c();P(t)});
